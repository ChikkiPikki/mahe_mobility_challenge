...
: rviz_common::Panel(parent)
{
  ...
  // Connect the signal triggered by the ROS thread to the Qt GUI thread slot
...
{
  // Get an rclcpp node specific to this RViz panel
  ...
  for (const auto & marker : msg->markers) {
    if (marker.action == visualization_msgs::msg::Marker::ADD) {
      if (marker.ns == "aruco" || marker.ns == "sign") {
        ...
        // Pass the data safely to the GUI thread; addRowToTable will update it if it exists
...
void MissionControlPanel::addRowToTable(const QString& type, int id, double x, double y, double z)
{
  ...
  // Check if a row with this type and id already exists
