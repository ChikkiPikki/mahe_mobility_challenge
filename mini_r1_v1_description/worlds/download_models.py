#!/usr/bin/env python3
"""
Run this once before launching your Gazebo world.
It will:
  ...
  4. Add GZ_SIM_RESOURCE_PATH and GZ_SYSTEM_PLUGIN_PATH to ~/.bashrc
  5. Apply those env vars to the current shell via a sourced script

Usage:
    python3 download_sdf_models.py <path_to_world.sdf_or_building.yaml>
"""
...
FUEL_CACHE     = (Path.home() / ".gz/fuel/fuel.gazebosim.org" / FUEL_OWNER.lower() / "models").resolve()
...
def get_models_from_sdf(path: Path) -> list[str]:
    # Match everything after model:// up to < or quote — preserves spaces in names
    raw = re.findall(r"model://([^<\"']+)", path.read_text())
    models = [m.strip() for m in raw if m.strip()]
    return sorted(set(models))

def get_models_from_yaml(path: Path) -> list[str]:
...
def get_model_names(path: Path) -> list[str]:
...
def find_versioned_dir(model_name: str) -> Path | None:
...
def ensure_symlink(model_name: str) -> bool:
    ...
    if not versioned:
        print(f"  ✗ No versioned dir found for '{model_name}' — download may have failed")
        return False
...
def download_model(model_name: str) -> bool:
    ...
    print(f"  gz fuel download --url {url}")
    r = subprocess.run(
        ["gz", "fuel", "download", "--url", url],
...
def main():
    ...
    print(f"    source ~/.bashrc && gz sim {input_path}\n")
